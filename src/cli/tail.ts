        ...
        try {
            ...
            if (data && data.length > 0) {
                data.reverse().forEach((log: any) => {
                    ...
                    console.log(
                        `[${time}] ${log.target.padEnd(15)} | ${color}${log.verdict.padEnd(10)}${reset} | Score: ${log.trust_score.toString().padEnd(3)} | Reason: ${log.reason}`
                    );
                });
            }
        } catch (e: any) {
            logger.error('[CLI] Tail error:', e.message);
        }
    }, 3000);
}
