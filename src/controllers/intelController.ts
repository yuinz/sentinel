...
export const preCheck: RequestHandler = async (req: Request, res: Response) => {
    ...
    const headerMock = req.headers['x-sentinel-mock-ip'] as string;
    const queryMock = req.query.mock_ip as string;
    ...
    logger.info(`[TRACE_PRECHECK] Target: ${target} | Mocked: ${!!mockIp} | Source: ${req.ip} | Req-UA: ${req.headers['user-agent']?.substring(0, 30)}`);
    ...
    return res.json({
        ...
        target: target,
        trace_id: crypto.randomUUID().substring(0, 8)
    });
};
...
export const verifyChallenge = async (req: AuthRequest, res: Response) => {
    ...
    const trustToken = IntelService.generateTrustToken(finalTarget);
    return res.json({
        success: true,
